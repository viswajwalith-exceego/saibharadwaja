function Contact() {
  return (
    <div>
      <div className="container text-center my-3">
        <span className="PageHeadingBS1">Contacts</span>
        <div className="mt-3">
          <img className="img-fluid" src="/images/pages/contactspage.png" alt="Contacts" />
        </div>
      </div>

      <div className="container bg-transparant p-sm-2 p-md-5">
        <div className="container bg-transparant d-flex textJustifiedNoMargin justify-content-center">
          <div className="container">
            <div className="row row-cols-1 row-cols-md-3">
              <div className="col p-2">
                <b>Parama Pujya Acharya Sri Ekkirala Bharadwaja Samadhi Mandiram</b> <br />
                Sri Shirdi Saibaba Temple,
                <br />
                Lawyerpet Extn.,
                <br />
                Mungamuru Donka,
                <br />
                Ongole-523001.
                <br />
              </div>

              <div className="col p-2">
                <b>Parama Pujya Divyajanani Alivelu Mangamma Samadhi Mandiram</b>
                <br />
                Bhatla Machavaram Village, Gummalampadu Village
                <br />
                Santhanuthalapadu Mandal,
                <br />
                Ongole-Podili-Kurnool Road,
                <br />
                Ongole- 523225.
                <br />
              </div>

              <div className="col p-2">
                <b>Parama Pujya Acharya Sri Ekkirala Bharadwaja Tapovanam</b>
                <br />
                Vidyanagar,
                <br />
                Nellore Dt.,
                <br />
                Pin - 524413.
              </div>

              <div className="col p-2">
                <b>Sri Manga Bharadwaja Trust Office</b>
                <br />
                Sri Manga Bharadwaja Trust,
                <br />
                Kothapeta,
                <br />
                Kondaiah Banku street,
                <br />
                Lawyerpet,
                <br />
                Ongole 523002.
                <br />
                Ph: 08592-233271
              </div>

              <div className="col p-2">
                <b>Sri Manga Bharadwaja Trust Office</b>
                <br />
                12-1-170/46P, Hanuman Nagar Jaipuri Colony,
                <br />
                Nagole, Hyderabad,
                <br />
                India - 500068.
                <br />
                Phone: +91-74160 41550
              </div>

              <div className="col p-2">
                <b>Saibharadwaja.org Admin Contact</b>
                <br />
                admincc@saibharadwaja.org
              </div>
            </div>
          </div>
        </div>
      </div>
      <br />
      <br />
      <br />
      <p className="jaisaimaster">Jai Sai Master</p>
      <br />
      <br />
      <br />
    </div>
  )
}

export default Contact

